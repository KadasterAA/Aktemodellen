...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_hypotheek_particulier-3.0.0.xml')" />
		...
		<xsl:variable name="genderShouldBePrinted">
			<xsl:choose>
				...
				<xsl:when test="not(part:tia_Gegevens/part:IMKAD_KadNatuurlijkPersoon)
						...
						or normalize-space(translate(part:tia_Gegevens/part:GBA_Ingezetene/part:geslacht/part:geslachtsaanduiding, $upper, $lower)) = 'onbekend')">
					<xsl:text>false</xsl:text>
				</xsl:when>
				<xsl:when test="not(part:tia_Gegevens/part:IMKAD_KadNatuurlijkPersoon)
						...
						or normalize-space(translate(part:tia_Gegevens/part:IMKAD_NietIngezetene/part:geslacht, $upper, $lower)) = 'onbekend')">
					<xsl:text>false</xsl:text>
				</xsl:when>
				<xsl:otherwise>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="part:GBA_Ingezetene | part:IMKAD_NietIngezetene" mode="do-gender-salutation-part">
		<xsl:choose>
			...
			<xsl:when test="translate(part:geslacht/part:geslachtsaanduiding, $upper, $lower) = 'onbekend' or translate(part:geslacht, $upper, $lower) = 'onbekend'">
				<xsl:text/>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
