...
<!--
*********************************************************
Stylesheet: keuzeblok_partijnamen_hypotheekakte_obv.xsl
Version: 1.0.0
*********************************************************
Description:
KEUZEBLOK PARTIJNAMEN HYPOTHEEKAKTE - Obvion
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Partij" mode="do-mortgage-deed-party-name">
		...
		<xsl:choose>
			...
			<xsl:when test="translate(tia:aanduidingPartij, $upper, $lower) = 'geldnemer of hypotheekgever'">
				...
				<xsl:text>ook te noemen: ‘geldnemer of hypotheekgever’</xsl:text>
			</xsl:when>
			...
			<xsl:when test="translate(tia:aanduidingPartij, $upper, $lower) = 'aanduiding per persoon'">
				...
				<xsl:if test="(not($existGevolmachtigde) and $number) or $name">
					...
					<u><xsl:text>geldnemer</xsl:text></u>
					<xsl:text>' en </xsl:text>
					<xsl:if test="$number">
						<xsl:text>de verschenen </xsl:text>
						<xsl:choose>
					...
					<xsl:text>ook te noemen: '</xsl:text>
					<u><xsl:text>hypotheekgever</xsl:text></u>
					<xsl:text>'</xsl:text>
				</xsl:if>
			</xsl:when>
