		...
		<xsl:choose>
			<xsl:when test="tia:IMKAD_AangebodenStuk/tia:Partij/tia:Partij">
				...
				<xsl:for-each select="$partyRestructured/tia:IMKAD_AangebodenStuk/tia:Partij">
					...
					<xsl:variable name="numberOfPersonsInFirstNestedParty" select="count($parties[$position]/tia:Partij[1]/descendant::tia:IMKAD_Persoon)"/>
					<xsl:variable name="numberOfPersonsInSecondNestedParty" select="count($parties[$position]/tia:Partij[2]/descendant::tia:IMKAD_Persoon)"/>
		...
		<table cellspacing="0" cellpadding="0">
			<tbody>
				...
				<tr>
					...
					<td valign="top">
						<xsl:text>de betaling van de rente (inclusief overeen te komen verhogingen), boeten,  
 vertragingsrente, kosten, schadevergoedingen nu of in de toekomst aan ASR verschuldigd 
 uit hoofde van de Leningovereenkomst en de betaling van al hetgeen ASR overigens uit 
 hoofde van de Leningovereenkomst, deze akte of de Algemene Voorwaarden van de 
 Schuldenaar te vorderen mocht hebben, welke in deze paragraaf b bedoelde bedragen 
 gezamenlijk worden begroot op een bedrag ad </xsl:text>
						<xsl:call-template name="amountText">
	...
	<xsl:template match="tia:Partij" mode="do-comparitie-nestedPartij">
													...
													<tbody>
														<tr>
															...
															<td>
																...
																<xsl:if test="$numberOfPersonsInFirstNestedParty > 1">
												...
												<table>
													<tbody>
														<tr>
															...
															<td>
																<xsl:text>hierna</xsl:text>
																<xsl:if test="$numberOfPersonsInSecondNestedParty > 1">
																	<xsl:text>, zowel tezamen als ieder afzonderlijk,</xsl:text>
																</xsl:if>
		...
		<xsl:if test="($numberOfPersonsInFirstNestedParty + $numberOfPersonsInSecondNestedParty) = 0">
			<p style="margin-left:30px">
				<xsl:text>hierna ook te noemen: "ASR";</xsl:text>
			</p>
		</xsl:if>
	</xsl:template>
