...
<!--
*************************************************************
Stylesheet: hypotheek_obvion.xsl
Version: 7.0.0
- [AA-8869] Obvion: Afsluiting hypotheekgever partij is verkeerd
- [AA-8835] Aanpassing Obvion: alleen oppakken wanneer we er een wijziging is op bankmodel
*************************************************************
Public:
(mode) do-deed
-->
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" exclude-result-prefixes="tia xsl xlink kef gc" version="1.0">
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_hypotheek_obvion-6.0.0.xml')"/>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Bericht_TIA_Stuk" mode="do-deed">
		...
		<xsl:variable name="personenVerklaren" select="tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_personenverklaren']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
				translate(normalize-space($keuzeteksten/*/tia:TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_personenverklaren']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
				translate(normalize-space(current()/tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_personenverklaren']/tia:tekst), $upper, $lower)]), $upper, $lower)]"/>

		<xsl:variable name="verklaren" select="tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaren']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
				translate(normalize-space($keuzeteksten/*/tia:TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaren']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
				translate(normalize-space(current()/tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaren']/tia:tekst), $upper, $lower)]), $upper, $lower)]"/>

		<xsl:variable name="comparantenOfPersonen" select="tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_comparantenofpersonen']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
				translate(normalize-space($keuzeteksten/*/tia:TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_comparantenofpersonen']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
				translate(normalize-space(current()/tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_comparantenofpersonen']/tia:tekst), $upper, $lower)]), $upper, $lower)]"/>
		...
		<p>
			...
			<xsl:value-of select="$comparantenOfPersonen"/>
			<xsl:text> verklaren dat hypotheekgever en geldgever zijn overeengekomen en zo nodig komen zij hierbij overeen dat door hypotheekgever ten behoeve van geldgever tot zekerheid als in deze akte omschreven het recht van hypotheek en pandrechten zal worden gevestigd op de in deze akte en na te melden Algemene Voorwaarden omschreven goederen. Deze overeenkomst blijkt uit een aan deze akte gehechte, door hypotheekgever geaccepteerde, bindend aanbod ter uitvoering waarvan het navolgende is overeengekomen:</xsl:text>
		</p>
		<!-- Details of Mortgage -->
				...
				<table cellspacing="0" cellpadding="0">
					<tbody>
						...
						<td>
							...
							<xsl:value-of select="$personenVerklaren"/>
							<xsl:text> sub 2 (en/of hypotheekgever) </xsl:text>
							<xsl:value-of select="$verklaren"/>
	...
	**** Overbruggingshypotheek      ********************************************************************************
	...
	<xsl:template match="tia:StukdeelHypotheek" mode="do-overbruggingshypotheek">
		<tr>
			...
			<td class="number" valign="top">
				<u><xsl:text>OVERBRUGGINGSHYPOTHEEK</xsl:text></u>
			</td>
		</tr><tr>
		<td class="number" valign="top">
			<xsl:text>&#xFEFF;</xsl:text>
