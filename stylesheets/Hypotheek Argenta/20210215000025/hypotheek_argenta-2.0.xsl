		...
		<p>
			<xsl:choose>
				<xsl:when test="$partijVerzekeraarAanwezig='true'">
					<xsl:text>Van het bestaan van de volmacht aan de comparanten onder 2. genoemd is mij, notaris, genoegzaam gebleken.</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>Van het bestaan van de mondelinge volmacht aan de comparant onder 2. genoemd is mij, notaris, genoegzaam gebleken.</xsl:text>
				</xsl:otherwise>
			</xsl:choose>
		</p>
		<p>
