...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" exclude-result-prefixes="tia xsl exslt xlink kef gc" version="1.0">
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_hypotheek_argenta-2.0.0.xml')"/>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Bericht_TIA_Stuk" mode="do-deed">
		...
		<xsl:variable name="mondelingeVolmacht" select="tia:IMKAD_AangebodenStuk/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_mondelinge']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
		translate(normalize-space($keuzeteksten/*/tia:TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_mondelinge']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
		translate(normalize-space(current()/tia:IMKAD_AangebodenStuk/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_mondelinge']/tia:tekst), $upper, $lower)]), $upper, $lower)]"/>
		...
		<p>
			...
			<xsl:if test="$mondelingeVolmacht = 'true'">
				<xsl:text>mondelinge </xsl:text>
			</xsl:if>
			<xsl:text>volmacht aan de </xsl:text>
		...
		<table cellspacing="0" cellpadding="0">
			<tbody>
				...
				<xsl:variable name="en">
					<xsl:choose>
						<xsl:when test="string-length($hypotheekStelling2) != 0">
				...
				<xsl:call-template name="do-hypotheekstelling">
					<xsl:with-param name="stukdeel" select="$hypotheekStelling1"/>
					<xsl:with-param name="volgnummer" select="'1'"/>
					<xsl:with-param name="en" select="$en"/>
					<xsl:with-param name="minus" select="$hypotheekStelling2"/>
				</xsl:call-template>
	
				<xsl:if test="string-length($hypotheekStelling2) != 0">
				<xsl:call-template name="do-hypotheekstelling">
					<xsl:with-param name="stukdeel" select="$hypotheekStelling2"/>
					<xsl:with-param name="volgnummer" select="'2'"/>
					<xsl:with-param name="minus" select="$hypotheekStelling2"/>
				</xsl:call-template>			
				</xsl:if>
			</tbody>
	...
	<xsl:template name="do-hypotheekstelling">
		...
		<xsl:param name="en" select="''"/>
			<tr>
					<td class="number" valign="top">
					<xsl:choose>
						<xsl:when test="string-length($minus) != 0">
							<xsl:text>-</xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<xsl:text></xsl:text>
					...
					<td>
						<xsl:text>hierbij aan geldverstrekker te verlenen, die van geldnemer aanvaardt, op het hierna te omschrijven registergoed genoemd onder A.2. Registergoed </xsl:text>
						<xsl:value-of select="$volgnummer"/>
						<xsl:text> het recht van </xsl:text>
						<xsl:value-of select="kef:convertOrdinalToText($stukdeel/tia:rangordeHypotheek)"/>
				...
				<tr>
					...
					<td>
						...
						<xsl:call-template name="amountText">
							<xsl:with-param name="amount" select="$stukdeel/tia:hoofdsom/tia:som"/>
							<xsl:with-param name="valuta" select="$stukdeel/tia:hoofdsom/tia:valuta"/>
						</xsl:call-template>
						<xsl:text> </xsl:text>
						<xsl:call-template name="amountNumber">
							<xsl:with-param name="amount" select="$stukdeel/tia:hoofdsom/tia:som"/>
				...
				<tr>
					...
					<td>
						...
						<xsl:value-of select="kef:convertNumberToText($stukdeel/tia:tekstkeuze[tia:tagNaam='k_Rentepercentage']/tia:tekst)"/>
						<xsl:text> procent (</xsl:text>
						<xsl:value-of select="$stukdeel/tia:tekstkeuze[tia:tagNaam='k_Rentepercentage']/tia:tekst"/>
						<xsl:text>%) van het bedrag hiervoor onder 1., dat is </xsl:text>
						<xsl:call-template name="amountText">
							<xsl:with-param name="amount" select="$stukdeel/tia:bedragRente/tia:som"/>
							<xsl:with-param name="valuta" select="$stukdeel/tia:bedragRente/tia:valuta"/>
						</xsl:call-template>
						<xsl:text> </xsl:text>
						<xsl:call-template name="amountNumber">
							<xsl:with-param name="amount" select="$stukdeel/tia:bedragRente/tia:som"/>
				...
				<tr>
					<td valign="top" colspan="2">
						<xsl:text>een en ander derhalve tot een totaalbedrag ter grootte van </xsl:text>
						<xsl:call-template name="amountText">
							<xsl:with-param name="amount" select="$stukdeel/tia:bedragTotaal/tia:som"/>
							<xsl:with-param name="valuta" select="$stukdeel/tia:bedragTotaal/tia:valuta"/>
						</xsl:call-template>
						<xsl:text> </xsl:text>
						<xsl:call-template name="amountNumber">
							<xsl:with-param name="amount" select="$stukdeel/tia:bedragTotaal/tia:som"/>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:StukdeelHypotheek" mode="do-rights">
		...
		<p><xsl:text>Registergoed </xsl:text><xsl:value-of select="$volgnummer"/></p>
		<xsl:choose>
