...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common" xmlns:xlink="http://www.w3.org/1999/xlink" exclude-result-prefixes="tia kef gc xsl exslt xlink" version="1.0">
	...
	*********************************************************
	...
	<xsl:template match="tia:Partij" mode="do-vervreemderRechtRef-party">
		...
		<xsl:variable name="aantalVoormalig" select="count(tia:IMKAD_Persoon//tia:VoormaligeRechtspersoon)"/>

<!--		<p>
			<xsl:text>aantalRefPartijen: </xsl:text>
			<xsl:value-of select="$aantalRefPartijen"/>
			<xsl:text>posSub: </xsl:text>
			<xsl:value-of select="$posSub"/>
			<xsl:text> - aantalSubpartijen: </xsl:text>
			<xsl:value-of select="$aantalSubpartijen"/>
			<xsl:text> - partijNummer: </xsl:text>
			<xsl:value-of select="$partijNummer"/>
			<xsl:text> - gerelateerdePersonen: </xsl:text>
			<xsl:value-of select="$gerelateerdePersonen"/>
			<xsl:text> - aantalVoormalig: </xsl:text>
			<xsl:value-of select="$aantalVoormalig"/>
		</p>
-->
		<!-- bepaal de opsomming (nummering) v/d volmachtgevers -->
		<xsl:choose>
			<!-- variant 1: slechts 1 gerefereerde volmachtgever -->
			<xsl:when test="($aantalSubpartijen = 1) and ($gerelateerdePersonen = 0)">
				<!-- vaste tekst -->
				<xsl:text> volmachtgever</xsl:text>
			</xsl:when>
			
			<!-- variant 2: twee gerefereerde volmachtgevers, die samen genummerd zijn -->
			<xsl:when test="($aantalSubpartijen = 1) and ($gerelateerdePersonen = 1)">
				<!-- vaste tekst -->
				<xsl:text> volmachtgever sub 1 en volmachtgever sub 2 </xsl:text>
			</xsl:when>

			<!-- variant 3: één volmachtgever met gerelateerde volmachtgevers, die NIET samen genummerd zijn -->
			<xsl:when test="(($aantalSubpartijen = 1) and ($gerelateerdePersonen >1))">
				<!-- vaste tekst -->
				<xsl:text> volmachtgever sub 1</xsl:text>
				<!-- hier na de eerste volmachtgever altijd een komma, omdat er nog min. twee gerelateerde volmachtgevers volgen -->
				<xsl:text>, </xsl:text>
				<!-- opsommen van ALLE gerelateerde PartijNummers -->
				<xsl:for-each select="tia:IMKAD_Persoon/tia:GerelateerdPersoon">
					<!-- vaste tekst -->
					<xsl:text> volmachtgever sub </xsl:text>
					<!-- partijNummer v/d GERELATEERDE volmachtgever opnoemen -->
			...
			<!-- variant 4: meerdere volmachtgevers die WEL of NIET samen genummerd zijn -->
			<xsl:when test="($aantalSubpartijen > 1)">
				<!-- vaste tekst -->
				<xsl:text> volmachtgever sub </xsl:text>
	...
	*********************************************************
	...
	<xsl:template match="tia:Partij" mode="do-verkrijgerRechtRef-party">
		...
		<xsl:choose>
			...
			<!-- variant 1: slechts 1 gerefereerde rechthebbende, dus geen opsomming -->
			<xsl:when test="($aantalSubpartijen = 1) and ($gerelateerdePersonen = 0)">
				<xsl:text> rechthebbende</xsl:text>
			</xsl:when>

			<!-- variant 2: twee gerefereerde rechthebbenden, die samen genummerd zijn -->
			<xsl:when test="($aantalSubpartijen = 1) and ($gerelateerdePersonen = 1)">
				<xsl:text> rechthebbende sub 1 en rechthebbende sub 2</xsl:text>
			</xsl:when>

			<!-- variant 4: 1 persoon met meerdere gerelateerde personen -->
			<xsl:when test="($aantalSubpartijen = 1) and ($gerelateerdePersonen > 1)">
			...
			<!-- variant 3: meerdere rechthebbenden die (niet) samen genummerd zijn -->
			<xsl:when test="($aantalSubpartijen > 1)">
				<!-- vaste tekst -->
				<xsl:text> rechthebbende sub </xsl:text>
				<!-- partijNummer v/d rechthebbende opnoemen -->
				<xsl:value-of select="$partijNummer"/>
				<!-- bepaal of niet de laatste opgenoemde volmachtgever is -->
				<xsl:choose>
					<xsl:when test="$posSub &lt; ($aantalRefPartijen - 1)">
						<xsl:text>,</xsl:text>
					</xsl:when>
	...
	*********************************************************
	...
	-->
	<xsl:template match="tia:VoormaligeRechtspersoon" mode="do-rechtsvorm">
