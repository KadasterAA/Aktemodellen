...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" exclude-result-prefixes="tia xsl xlink kef gc" version="1.0">
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_hypotheek_regiobank-1.00.xml')"/>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Bericht_TIA_Stuk" mode="do-deed">
			...
			<tbody>
				<tr>
					...
					<td>
						...
						<xsl:text> en zoals deze is of zal worden vastgelegd in een door de bank en de schuldenaar gesloten respectievelijk te sluiten overeenkomst van geldlening RegioBank (particulier) of zoals deze eventueel, al dan niet met kredietfaciliteiten, later gewijzigd, aangevuld of verhoogd mocht worden;</xsl:text>
					</td>
				</tr>
				<tr>
					<td class="number" valign="top">
		...
		<xsl:if test="tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[translate(tia:aanduidingHypotheek, $upper, $lower) = 'overbruggingshypotheek']">
			...
			<p>
				<xsl:text>Voorts verleent de schuldenaar, tot meerdere zekerheid voor de betaling van de lening als hierboven vermeld, bij deze aan de bank, die van de schuldenaar aanvaardt het recht van </xsl:text>
	...
	<xsl:template match="tia:Partij" mode="do-parties">
		...
		<xsl:choose>
			...
			<xsl:when test="position() = 2">
				<p style="margin-left:30px">
					<xsl:text>hierna zowel RegioBank N.V. als haar rechtsopvolgers onder algemene of bijzondere titel te noemen:  '</xsl:text>
					<u>
