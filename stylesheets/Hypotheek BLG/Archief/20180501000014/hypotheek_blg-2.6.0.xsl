...
<!--
*************************************************************
Stylesheet: hypotheek_blg.xsl
Version: 2.6.0 (AA-3748: keuzeteksten_hypotheek_blg-2.3.0.xml, tekstblok burgerlijke staat))
Date: 2018-04-06
*************************************************************
...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" exclude-result-prefixes="tia xsl xlink kef gc" version="1.0">
	...
	<!-- blg specific global variables -->
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_hypotheek_blg-2.3.0.xml')"/>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Bericht_TIA_Stuk" mode="do-deed">
		...
		<p>
			<xsl:text>De comparanten verklaarden, dat door de hypotheekbank aan de geldnemer overeenkomstig een tussen hen gesloten overeenkomst van geldlening - welke overeenkomst is omschreven in de door geldnemer geaccepteerde offerte van de hypotheekbank onder nummer </xsl:text><xsl:value-of select="tia:IMKAD_AangebodenStuk/tia:tia_TekstKeuze[tia:tagNaam='k_Offertenummer']/tia:tekst"/><xsl:text> - met ingang van heden een lening is verstrekt van </xsl:text>
			...
			<xsl:if test="tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[translate(tia:aanduidingHypotheek, $upper, $lower) = 'overbruggingshypotheek']/tia:bedragLening">
				<xsl:text>, en een overbruggingslening van </xsl:text>
				<xsl:call-template name="amountText">
			...
			<xsl:text> (waarvoor de geldnemer aan de hypotheekbank kwijting verleent en welk bedrag hij aan de hypotheekbank wettig - en als er meerdere geldnemers zijn: hoofdelijk - schuldig is) waarbij hij zich heeft verplicht tot het vestigen ten behoeve van de hypotheekbank van recht van hypotheek en pand op de in deze akte omschreven registergoederen/zaken, met inachtneming van de hierna vermelde voorwaarden.</xsl:text>
			<br/>
			<xsl:text>Tot zekerheid voor de betaling van elke huidige of toekomstige schuld van de geldnemer aan de hypotheekbank op grond van deze lening en/of van in de toekomst door hem met de hypotheekbank overeen te komen leningen, of op grond van andere rechtsverhoudingen tussen beiden:</xsl:text>
		</p>
		<table cellspacing="0" cellpadding="0">
			<tbody>
				<tr>
					...
					<td>
						<xsl:text>voor de hoofdsom </xsl:text>
						<xsl:call-template name="amountText">
				...
				<tr>
					...
					<td>
						<xsl:text>voor daarover verschuldigde renten voor een periode van drie jaren en voor eventuele boeten en door de hypotheekbank te maken kosten, tezamen tot een bedrag van vijfenveertig procent (45%) van de hoofdsom, zijnde </xsl:text>
						<xsl:call-template name="amountText">
		...
		<p>
			<xsl:text>verklaarde de geldnemer tot het totaalbedrag van </xsl:text>
			<xsl:call-template name="amountText">
			...
			<xsl:text>hypotheek te verlenen aan de hypotheekbank op het hierna te omschrijven onderpand:</xsl:text>
		</p>
		<!-- Registered objects -->
		...
		<p>
			<xsl:text>De comparant onder 2 verklaarde deze rechten van hypotheek en pand namens de hypotheekbank aan te nemen.</xsl:text>
		</p>
				...
				<table cellpadding="0" cellspacing="0">
					<tbody>
						<xsl:call-template name="processRights">
							...
							<xsl:with-param name="haveAdditionalText" select="'.'"/>  <!-- forceer de . na de laatste aanroep recht/registergoed -->
						</xsl:call-template>
					</tbody>
				</table>
			</xsl:otherwise>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Partij" mode="do-parties">
		...
		<xsl:variable name="toestemmingpartners" select="tia:tekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_toestemmingpartners']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
				...
				translate(normalize-space(current()/tia:tekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_toestemmingpartners']/tia:tekst), $upper, $lower)]), $upper, $lower)]"/>
		<table cellspacing="0" cellpadding="0">
			<tbody>
		...
		<xsl:choose>
			<xsl:when test="position() = 1">
				<xsl:if test="@id = substring-after(../tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:vervreemderRechtRef/@xlink:href, '#') and $toestemmingpartners != ''">
					<p style="margin-left:30px">
						<xsl:value-of select="$toestemmingpartners"/>
						<xsl:text>,</xsl:text>
					</p>
				</xsl:if>
				<p style="margin-left:30px">
					...
					<xsl:text>te noemen 'geldnemer';</xsl:text>
				</p>
			</xsl:when>
			<xsl:when test="position() = 2">
				<p style="margin-left:30px">
					...
					<xsl:text> als haar rechtsopvolgers onder algemene of bijzondere titel te noemen: ‘de hypotheekbank’.</xsl:text>
				</p>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
