...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" exclude-result-prefixes="xsl exslt xlink kef" version="1.0">
	<xsl:template match="partner">
		...
		<xsl:variable name="aantalHuwelijk" select="count(eerdereBurgerlijkeStaat/huwelijkPartnerRef)"/>
		<xsl:variable name="aantalGeregistreerdPartner" select="count(eerdereBurgerlijkeStaat/geregistreerdPartnerRef)"/>
		<h3 class="header">
		...
		<p>
			...
			<xsl:value-of select="gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_tentijde']/tekst"/>
			<xsl:text> van </xsl:text>
			<xsl:value-of select="$zijnHaar"/>
			<xsl:text> overlijden </xsl:text>
			<xsl:choose>
				<xsl:when test="translate(gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_burgerlijkestaat']/tekst,$upper, $lower) = 'huwelijk'">
		...
		<p>
			...
			<xsl:value-of select="gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_burgerlijkestaat']/tekst"/>
			<xsl:text> is ontbonden door </xsl:text>
			<xsl:value-of select="gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_overlijden']/tekst"/>
			<xsl:text>.</xsl:text>
		</p>
		<xsl:if test="eerdereBurgerlijkeStaat">
			<p>
				<xsl:text>De </xsl:text>
				<xsl:value-of select="$overledeneAanduiding"/>
			...
			<table cellspacing="0" cellpadding="0">
				<tbody>
					<xsl:for-each select="eerdereBurgerlijkeStaat">
						<xsl:variable name="eerderePartnerRef" select="huwelijkPartnerRef/@xlink:href | geregistreerdPartnerRef/@xlink:href"/>
						...
						<xsl:choose>
							<xsl:when test="$aantalHuwelijk + $aantalGeregistreerdPartner = 1">
								<tr>
									<td>
										<xsl:call-template name="tb-NatuurlijkPersoon">
											<xsl:with-param name="partner" select="$eerderePartner"/>
										</xsl:call-template>
										<xsl:text>.</xsl:text>
									</td>
								</tr>
		...
		<p>
			<xsl:choose>
				<xsl:when test="translate(gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_huwelijksvermogensrecht']/tekst,$upper, $lower) = 'algehele'">
				...
				<xsl:when test="translate(gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_huwelijksvermogensrecht']/tekst,$upper, $lower) = 'beperkte'">
		...
		<xsl:choose>
			<xsl:when test="gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_ontbondengemeenschap']/tekst = '1'">
				<p>
			...
			<xsl:when test="gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_ontbondengemeenschap']/tekst = '2'">
				<p>
					<xsl:text>Het Nederlandse huwelijksvermogensrecht was van toepassing. Zij hebben huwelijkse voorwaarden gemaakt</xsl:text>
					<xsl:if test="translate(gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_uitsluitinggemeenschapgoederen']/tekst,$upper, $lower) = 'true'">
			...
			<xsl:when test="gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_ontbondengemeenschap']/tekst = '3'">
				<p>
					<xsl:text>Het Nederlandse huwelijksvermogensrecht was van toepassing. Zij hebben partnerschapsvoorwaarden gemaakt</xsl:text>
					<xsl:choose>
						<xsl:when test="translate(gegevensPartner/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_uitsluitinggemeenschapgoederen']/tekst,$upper, $lower) = 'true'">
							<xsl:text> waarbij elke gemeenschap van goederen is uitgesloten.</xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<xsl:text>.</xsl:text>
		...
		<xsl:choose>
			<xsl:when test="partnerRef">
				<xsl:choose>
					...
					<xsl:when test="translate(tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_burgerlijkestaat']/tekst,$upper, $lower) = 'geregistreerdpartnerschap'">
	...
	<xsl:template name="huwelijkOntbonden">
		...
		<xsl:if test="translate(tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_ontbondenhuwelijk']/tekst,$upper, $lower) = 'overlijden van laatstgenoemde op'">
			...
			<xsl:value-of select="kef:convertDateToText(datumOntbonden)"/>
			<xsl:text> te </xsl:text>
			<xsl:value-of select="plaatsOntbonden"/>
		</xsl:if>
		<xsl:text>, aangetekend in de registers van de burgerlijke stand van de gemeente </xsl:text>
		<xsl:value-of select="naamGemeente"/>
		<xsl:text> op </xsl:text>
