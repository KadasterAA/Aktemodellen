...
xmlns:vve="http://www.kadaster.nl/schemas/AA/VVE-WettelijkeVerdeling" 
xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" exclude-result-prefixes="vve kef xsl xlink" version="1.0">
	...
	<xsl:include href="keuzeblok_vve_wettelijkeverdeling_aanvaarding-1.0.xsl"/>
	...
	<xsl:include href="keuzeblok_vve_wettelijkeverdeling_partner-1.0.xsl"/>
	<xsl:include href="keuzeblok_vve_wettelijkeverdeling_afstammelingen-1.0.xsl"/>
	<xsl:include href="keuzeblok_vve_wettelijkeverdeling_testament_uiterste_wilsbeschikking-1.0.xsl"/>
	<xsl:include href="keuzeblok_vve_wettelijkeverdeling_erfdelen-1.0.xsl"/>
	...
	<xsl:include href="tekstblok_aanhef_nieuw-1.00.xsl"/>
	<xsl:include href="tekstblok_equivalentieverklaring_nieuw-1.00.xsl"/>
	<xsl:include href="tekstblok_natuurlijk_persoon_nieuw-1.00.xsl"/>
	<xsl:include href="tekstblok_personalia_van_natuurlijk_persoon_nieuw-1.00.xsl"/>
	<xsl:include href="tekstblok_woonadres_nieuw-1.00.xsl"/>
	
	<!-- Common global variables -->
	<xsl:variable name="upper" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ'"/>
	<xsl:variable name="lower" select="'abcdefghijklmnopqrstuvwxyz'"/>
	<xsl:variable name="documentTitle" select="vve:Bericht_TIA_Stuk/IMKAD_AangebodenStuk/titelAkte"/>
	<xsl:variable name="overledeneAanduiding" select="vve:Bericht_TIA_Stuk/IMKAD_AangebodenStuk/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_overledeneaanduiding']/tekst"/>
	<xsl:variable name="benamingPartner" select="vve:Bericht_TIA_Stuk/IMKAD_AangebodenStuk/stukdeelVVE/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_benamingpartner']/tekst"/>
	<xsl:variable name="blijkens" select="vve:Bericht_TIA_Stuk/IMKAD_AangebodenStuk/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_blijkens']/tekst"/>
	<xsl:variable name="blijkens_up" select="concat(translate(substring($blijkens,1,1),$lower,$upper),substring($blijkens,2))"/>
	<xsl:variable name="blijkens_low" select="concat(translate(substring($blijkens,1,1),$upper,$lower),substring($blijkens,2))"/>
	<xsl:variable name="opsommingPersonen" select="vve:Bericht_TIA_Stuk/IMKAD_AangebodenStuk/opsommingPersonen"/>
	<xsl:variable name="overledeneRef" select="vve:Bericht_TIA_Stuk/IMKAD_AangebodenStuk/stukdeelVVE/overlijden/overledeneRef/@xlink:href"/>
	<xsl:variable name="overledenPersoon" select="$opsommingPersonen/IMKAD_Persoon[@id = substring-after($overledeneRef,'#')]"/>
	<xsl:variable name="benamingTestament" select="vve:Bericht_TIA_Stuk/IMKAD_AangebodenStuk/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_benamingtestament']/tekst"/>
	...
	<xsl:variable name="partnerRef" select="vve:Bericht_TIA_Stuk/IMKAD_AangebodenStuk/stukdeelVVE/partner/gegevensPartner/partnerRef/@xlink:href"/>
	...
	<xsl:apply-templates select="IMKAD_AangebodenStuk" mode="do-statement-of-equivalence"/>
		...
		<xsl:apply-templates select="IMKAD_AangebodenStuk/stukdeelVVE/partner"/>
		...
		<p>
			...
			<xsl:value-of select="IMKAD_AangebodenStuk/stukdeelVVE/tekstkeuze[translate(tagNaam, $upper, $lower) = 'k_toepasselijkrecht']/tekst"/>
			<xsl:text>, Nederlands recht van toepassing.</xsl:text>
		</p>
		<xsl:apply-templates select="IMKAD_AangebodenStuk/stukdeelVVE/erfdelen"/>
		<xsl:apply-templates select="IMKAD_AangebodenStuk/stukdeelVVE/wettelijkeVerdeling"/>
		<xsl:apply-templates select="IMKAD_AangebodenStuk/stukdeelVVE/aanvaarding"/>
	...
	<xsl:template name="tb-NatuurlijkPersoon">
		...
		<xsl:apply-templates select="$partner" mode="do-natural-person"/>
		<xsl:if test="$toonWoonplaats = 'true'">
			<xsl:text>, wonende te </xsl:text>
			<xsl:apply-templates select="$partner/IMKAD_WoonlocatiePersoon" mode="do-address"/>
		</xsl:if>
	</xsl:template>
