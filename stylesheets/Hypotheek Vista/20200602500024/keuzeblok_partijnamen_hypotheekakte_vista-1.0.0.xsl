...
<!--
*********************************************************
Stylesheet: keuzeblok_partijnamen_hypotheekakte_vista.xsl
Version: 1.0.0
*********************************************************
Description:
KEUZEBLOK PARTIJNAMEN HYPOTHEEKAKTE - Vista
	...
	<xsl:template match="tia:Partij" mode="do-keuzeblok-partijnamen-hypotheekakte">
		...
		<xsl:choose>
			...
			<xsl:when test="translate(tia:aanduidingPartij, $upper, $lower) = 'geldnemer of hypotheekgever'">
				...
				<xsl:text> hierna te noemen: geldnemer of hypotheekgever</xsl:text>
			</xsl:when>
			...
			<xsl:when test="translate(tia:aanduidingPartij, $upper, $lower) = 'aanduiding per persoon'">
				...
				<xsl:if test="(not($existGevolmachtigde) and $number) or $name">
					...
					<xsl:text>hierna te noemen: geldnemer en </xsl:text>
					<xsl:if test="$number">
						<xsl:text>de verschenen </xsl:text>
						<xsl:choose>
