...
Version: 6.1.0
- [AA-8931] Bug: Rabobank en Particuliere Hypotheek XSD nog niet genderneutraal
*********************************************************
Description:
Hypotheek Rabobank Modeldocument versie 5.0.0
...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:rabo="http://www.kadaster.nl/schemas/KIK/RabobankHypotheekakte/v20180901" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" exclude-result-prefixes="tia rabo kef xsl xlink gc" version="1.0">
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_hypotheek_rabobank-5.0.0.xml')"/>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Bericht_TIA_Stuk" mode="do-deed">
		...
		<p>
			...
			<xsl:value-of select="tia:IMKAD_AangebodenStuk/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_personenvolmacht']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
		...
		<xsl:variable name="genderShouldBePrinted">
			<xsl:choose>
				...
				<xsl:when test="not(rabo:tia_Gegevens/rabo:IMKAD_KadNatuurlijkPersoon)
						...
						or normalize-space(translate(rabo:tia_Gegevens/rabo:GBA_Ingezetene/rabo:geslacht/rabo:geslachtsaanduiding, $upper, $lower)) = 'onbekend')">
					<xsl:text>false</xsl:text>
				</xsl:when>
				<xsl:when test="not(rabo:tia_Gegevens/rabo:IMKAD_KadNatuurlijkPersoon)
						...
						or normalize-space(translate(rabo:tia_Gegevens/rabo:IMKAD_NietIngezetene/rabo:geslacht, $upper, $lower)) = 'onbekend')">
					<xsl:text>false</xsl:text>
				</xsl:when>
				<xsl:otherwise>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="rabo:GBA_Ingezetene | rabo:IMKAD_NietIngezetene" mode="do-gender-salutation-rabo">
		<xsl:choose>
			...
			<xsl:when test="translate(rabo:geslacht/rabo:geslachtsaanduiding, $upper, $lower) = 'onbekend' or translate(rabo:geslacht, $upper, $lower) = 'onbekend'">
				<xsl:text/>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
