...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
	...
	exclude-result-prefixes="tia rabo kef xsl xlink"
	version="1.0">
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_hypotheek_rabobank-1.07.xml')"/>
	<xsl:variable name="documentTitle">
		<xsl:choose>
			...
			<xsl:otherwise><xsl:text>Rabobank Hypotheek</xsl:text></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Bericht_TIA_Stuk" mode="do-deed">
		...
		<!-- Document title -->
		<xsl:if test="normalize-space($documentTitle) != '' and translate($documentTitle, $upper, $lower) != 'rabobank hypotheek'">
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="rabo:IMKAD_Persoon" mode="do-natural-person-rabo">
		<xsl:apply-templates select="rabo:tia_Gegevens[1]/rabo:GBA_Ingezetene | rabo:tia_Gegevens[1]/rabo:IMKAD_NietIngezetene" mode="do-gender-salutation-rabo" />
		<xsl:text> </xsl:text>
		<xsl:choose>
			<xsl:when test="rabo:tia_Gegevens[2]/rabo:IMKAD_KadNatuurlijkPersoon">
				<xsl:apply-templates select="rabo:tia_Gegevens[2]/rabo:IMKAD_KadNatuurlijkPersoon" mode="do-person-data-rabo" />
				<xsl:text>, bij de gemeentelijke basisregistratie bekend als </xsl:text>
				<xsl:apply-templates select="rabo:tia_Gegevens[1]/rabo:GBA_Ingezetene" mode="do-person-data-rabo" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="rabo:tia_Gegevens[1]/rabo:GBA_Ingezetene | rabo:tia_Gegevens[1]/rabo:IMKAD_NietIngezetene" mode="do-person-data-rabo" />
			</xsl:otherwise>
		</xsl:choose>
		<xsl:text>, </xsl:text>
		<xsl:apply-templates select="rabo:tia_Gegevens[1]/rabo:GBA_Ingezetene | rabo:tia_Gegevens[1]/rabo:IMKAD_NietIngezetene" mode="do-birth-data-rabo" />
	</xsl:template>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="rabo:GBA_Ingezetene | rabo:IMKAD_NietIngezetene" mode="do-birth-data-rabo">
		<xsl:variable name="Datum_DATE" select="substring(string(rabo:geboorte/rabo:geboortedatum
			| rabo:geboortedatum), 0, 11)"/>
		<xsl:variable name="Datum_STRING">
			<xsl:if test="$Datum_DATE != ''">
				<xsl:value-of select="kef:convertDateToText($Datum_DATE)"/>
			</xsl:if>
