...
<!--
*************************************************************
Stylesheet: hypotheek_svenska-3.0.xsl
Version: 3.0
- [AA-9012] Nieuw modeldocument Svenska Handelsbanken
*************************************************************
...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:exslt="http://exslt.org/common" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" exclude-result-prefixes="tia xsl xlink kef gc" version="1.0">
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_hypotheek_svenska-3.0.0.xml')"/>
	...
	<xsl:variable name="consumentzakelijk"
				  select="tia:Bericht_TIA_Stuk/tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_consumentzakelijk']/tia:tekst"/>
	<xsl:variable name="hoofdstuk"
				  select="tia:Bericht_TIA_Stuk/tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_hoofdstuk']/tia:tekst"/>
	<!--
	*********************************************************
	Mode: do-deed
	*********************************************************
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Bericht_TIA_Stuk" mode="do-deed">
		...
		<tr>
			...
			<td valign="top">
				<strong>
					...
					<xsl:value-of select="$consumentzakelijk"/>
					<xsl:text>:</xsl:text>
				</strong>
				<xsl:text> de actuele versie van de Algemene Voorwaarden van Handelsbanken Nederland </xsl:text>
				<xsl:value-of select="$consumentzakelijk"/>
				<xsl:text>, zoals gedeponeerd bij de Kamer van Koophandel te Amsterdam en ook beschikbaar is gesteld op de website www.handelsbanken.nl, welke set voorwaarden aan de Onderzetter/Schuldenaar ter hand is gesteld;</xsl:text>
			</td>
		</tr>
		<tr>
			<td class="number" valign="top">
		...
		<p>
			<xsl:text>De comparanten, handelend als vermeld, verklaren dat de Onderzetter/Schuldenaar en Handelsbanken zijn overeengekomen, dan wel bij deze overeenkomen, dat de Onderzetter ten behoeve van Handelsbanken de rechten van hypotheek en pand op de in deze akte en op de in de Algemene Voorwaarden </xsl:text>
			<xsl:value-of select="$consumentzakelijk"/>
			<xsl:text> omschreven goederen zal vestigen, tot zekerheid als in deze akte vermeld.</xsl:text>
		</p>
		<p>
			<strong>
		...
		<p>
			<xsl:text>Ter uitvoering van hetgeen heden is overeengekomen en voormelde kredietovereenkomst</xsl:text>
			<xsl:if test="normalize-space($kredietovereenkomstMeervoud) = 'kredietovereenkomsten'">
				<xsl:text>en</xsl:text>
			</xsl:if>
			...
			<tbody>
				<tr>
					...
					<td colspan="2">
						...
						<xsl:text> hypotheek op (elk van) het (de) hierna onder "Omschrijving Registergoed" vermelde registergoed(eren), dat (die) verder zal (zullen) worden genoemd: het Registergoed, met al hetgeen daarvan naar verkeersopvatting onderdeel uitmaakt;</xsl:text>
					</td>
				</tr>
				<tr>
					<td class="number" valign="top">
				...
				<tr>
					...
					<td colspan="2">
						<xsl:text>een recht van pand op de rechten als bedoeld in </xsl:text>
						<xsl:choose>
							<xsl:when test="normalize-space($hoofdstuk) = 'hoofdstuk 9'"><xsl:text>hoofdstuk 9 artikel 2 van de Algemene Voorwaarden Zakelijk</xsl:text></xsl:when>
							<xsl:when test="normalize-space($hoofdstuk) = 'hoofdstuk 10'"><xsl:text>hoofdstuk 10 artikel 2 van de Algemene Voorwaarden Consument</xsl:text></xsl:when>
						</xsl:choose>
						<xsl:text>; en</xsl:text>
					</td>
				</tr>
	...
	<xsl:template match="tia:Partij" mode="do-keuzeblok-partijnamen-hypotheekakte">
		...
		<xsl:choose>
			...
			<xsl:when test="translate(tia:aanduidingPartij, $upper, $lower) = 'onderzetter/schuldenaar'">
				<xsl:text>voor zover in deze akte niet anders aangeduid, hierna</xsl:text>
