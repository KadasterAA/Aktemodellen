...
Version: 1.91 (JIRA: AA-2303)
Version: 1.91 (AA-2284)
*********************************************************
Description:
Deed of transfer.
					...
					<xsl:when test="$acquirerParty">
						<xsl:choose>
							...
							<xsl:otherwise>
								...
								<table cellspacing="0" cellpadding="0">
									<tbody>
										<xsl:for-each select="$acquirerParty/tia:IMKAD_Persoon[translate(tia:tia_IndGerechtigde, $upper, $lower) = 'true']">
											<xsl:variable name="position" select="position()"/>
											<xsl:variable name="last" select="last()"/>
											<tr>
	...
	<xsl:template match="tia:IMKAD_AangebodenStuk" mode="do-purchase-and-transfer-tweeleveringen">
		...
		<xsl:for-each select="tia:StukdeelLevering[count(preceding-sibling::tia:StukdeelLevering) mod 2 = 0]">
			...
			<xsl:variable name="undividedLevering">
				<xsl:choose>
					<xsl:when test="translate(../tia:StukdeelKoop[tia:tia_Volgnummer = current()/tia:tia_Volgnummer]/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_onverdeeld']/tia:tekst, $upper, $lower) = 'true'">
						<xsl:choose>
			...
			<xsl:variable name="undividedLeveringNext">
				<xsl:choose>
					<xsl:when test="translate($koopLeveringNext/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_onverdeeld']/tia:tekst, $upper, $lower) = 'true'">
						<xsl:choose>
							<xsl:when test="$numberOfBuyersLevering = 2">
								<xsl:text>onverdeelde </xsl:text>
							</xsl:when>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_AangebodenStuk" mode="do-purchase-and-transfer-verkooprechtenmetcessie">
		...
		<xsl:for-each select="tia:StukdeelLevering">
			...
			<xsl:variable name="undividedLevering">
				<xsl:choose>
					<xsl:when test="translate(../tia:StukdeelKoop[tia:tia_Volgnummer = $koopovereenkomstnummer]/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_onverdeeld']/tia:tekst, $upper, $lower) = 'true'">
						<xsl:choose>
							<xsl:when test="$numberOfBuyersLevering = 2">
								<xsl:text>onverdeelde </xsl:text>
							</xsl:when>
					...
					<xsl:if test="tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_inschrijvingkoopaanduiding'] and tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_inschrijvingkoopaanduiding']/tia:tekst = 'true'">
						...
						<xsl:choose>
							<xsl:when test="tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_inschrijvingkoopvolgnummer']/tia:tekst = 'true'">
								<xsl:value-of select="$koopAkteCapitalized"/>
								<xsl:text> </xsl:text>
								<xsl:value-of select="count(preceding-sibling::tia:StukdeelKoop[tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_inschrijvingkoop']/tia:tekst = 'true']) + 1"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$deHetKoopakteLevering"/>
							</xsl:otherwise>
