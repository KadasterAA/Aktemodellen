...
AA-2992 Doorhalingsmodel: romeinse nummering aanpassen in alfabetische nummering
AA-4469 Tekstuele wijzigingen modeldocument Doorhaling
*********************************************************
Description:
...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common" xmlns:xlink="http://www.w3.org/1999/xlink" exclude-result-prefixes="tia kef gc xsl exslt xlink" version="1.0">
	...
	<xsl:include href="tekstblok_partij_natuurlijk_persoon_doorhaling-2.1.0.xsl"/>
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_doorhaling_hypotheek-6.14.0.xml')"/>
	...
	<xsl:variable name="numberOfRechthebbendeParties" select="count(tia:Bericht_TIA_Stuk/tia:IMKAD_AangebodenStuk/tia:Partij[substring((translate(tia:aanduidingPartij, $upper, $lower)),1,13) = 'rechthebbende']/tia:Partij)"/>
	...
	*********************************************************
	...
	<xsl:template name="do-overige-verklaringen">
		...
		<xsl:if test="tia:IMKAD_AangebodenStuk/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_vorderingen']/tia:tekst = 'true'">
			<h3>
				<u>
		...
		<tr>
			<xsl:if test="($aantalStukdelen) &gt; 1">
				<td class="number" valign="top">
					<xsl:number value="$stukDeelNumber" format="a."/>
				</td>
			</xsl:if>
			<td>
				<xsl:text>De </xsl:text>
			...
			<td>
				...
				<xsl:for-each select="tia:vervreemderRechtRef">
					...
					<xsl:apply-templates select="/tia:Bericht_TIA_Stuk/tia:IMKAD_AangebodenStuk/tia:Partij[
							(substring((translate(tia:aanduidingPartij, $upper, $lower)),1,13) = 'volmachtgever') or
							(substring((translate(tia:aanduidingPartij, $upper, $lower)),1,15) = 'hypotheekhouder')
							]/tia:Partij[@id=$volmachtgeverId]" mode="do-vervreemderRechtRef-party">
		...
		<tr>
			...
			<td>
				...
				<xsl:for-each select="tia:verkrijgerRechtRef">
					...
					<xsl:apply-templates select="/tia:Bericht_TIA_Stuk/tia:IMKAD_AangebodenStuk/tia:Partij[substring((translate(tia:aanduidingPartij, $upper, $lower)),1,13) = 'rechthebbende']/tia:Partij[@id=$rechthebbendeId]" mode="do-verkrijgerRechtRef-party">
	...
	*********************************************************
	...
	<xsl:template match="tia:Partij" mode="do-vervreemderRechtRef-party">
		...
		<xsl:variable name="partner">
			<xsl:choose>
				<xsl:when test="translate(substring(tia:IMKAD_Persoon/tia:GerelateerdPersoon/tia:rol,1,7),$upper,$lower) = 'partner'">
					<xsl:text>true</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>false</xsl:text>
		...
		<xsl:variable name="huisgenoot">
			<xsl:choose>
				<xsl:when test="translate(substring(tia:IMKAD_Persoon/tia:GerelateerdPersoon/tia:rol,1,7),$upper,$lower) = 'huisgenoot'">
					<xsl:text>true</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>false</xsl:text>
		...
		<xsl:choose>
			...
			<xsl:when test="($aantalSubpartijen = 1) and ($gerelateerdePersonen = 1) and ($partner='true' or $huisgenoot='true')">
				<xsl:text> volmachtgevers </xsl:text>
			</xsl:when>
	...
	*********************************************************
	...
	<xsl:template name="aanvaardingVolmacht">
		...
		<xsl:choose>
			...
			<xsl:when test="tia:volmachtverleningAfstand/tia:aantalOnderhandseAkten != ''">
				<xsl:text> blijkt uit </xsl:text>
				<xsl:value-of select="kef:convertNumberToText(tia:volmachtverleningAfstand/tia:aantalOnderhandseAkten)"/>
				<xsl:text> onderhandse </xsl:text>
				<xsl:choose>
					<xsl:when test="tia:volmachtverleningAfstand/tia:aantalOnderhandseAkten = 1">
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_AangebodenStuk" mode="do-signature">
		...
		<xsl:if test="translate($type-document, $upper, $lower) = 'afschrift'">
			...
			<p>
				<xsl:if test="tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_afschrift']/tia:tekst = 'true'">
				...
				<xsl:if test="tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_kandidaattoegevoegd']">
					<xsl:choose>
						<xsl:when test="tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_kandidaattoegevoegd']/tia:tekst = 'kandidaat'">
							<xsl:text>kandidaat-</xsl:text>
						</xsl:when>
						<xsl:when test="tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_kandidaattoegevoegd']/tia:tekst = 'toegevoegd'">
