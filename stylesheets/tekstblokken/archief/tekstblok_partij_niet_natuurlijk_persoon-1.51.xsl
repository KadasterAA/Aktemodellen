									...
									<tr>
										...
										<td>
											...
											<xsl:choose>
												...
												<xsl:when test="position() = last()">
													<xsl:if test="$mainPerson/tia:IMKAD_PostlocatiePersoon">
														<xsl:text> (correspondentieadres voor alle aangelegenheden betreffende de hierna te vermelden rechtshandelingen: </xsl:text>
														<xsl:apply-templates select="$mainPerson/tia:IMKAD_PostlocatiePersoon" mode="do-correspondant-address" />
														<xsl:text>)</xsl:text>
													</xsl:if>
													<xsl:value-of select="$personTerminator" />
												</xsl:when>
									...
									<tr>
										...
										<td>
											...
											<xsl:choose>
												...
												<xsl:otherwise>
													<xsl:choose>
														<xsl:when test="$isLast = 'false' or $mainPersonInManagerContainsRelatedPersons = 'true'">
															<xsl:text>; en</xsl:text>
														</xsl:when>
														<xsl:otherwise>
															<xsl:text>; </xsl:text>
									...
									<tbody>
										<tr>
											...
											<td>
												...
												<xsl:choose>
													<xsl:when test="position() != last() or $isLast = 'false'">
														<xsl:text>; en</xsl:text>
													</xsl:when>
													<xsl:otherwise>
														<xsl:text>; </xsl:text>
			...
			<xsl:when test="$numberOfPersonsRepresentedByThisHoedanigheid = 1">
				<xsl:choose>
					...
					<xsl:when test="$actingInPrivateCurrentlyProcessedHoedanigheid">
						...
						<xsl:if test="$isThereMoreHoedanigheidsToProcess = 'false' and $mainPerson/tia:IMKAD_PostlocatiePersoon">
							<xsl:text> (correspondentieadres voor alle aangelegenheden betreffende de hierna te vermelden rechtshandelingen: </xsl:text>
							<xsl:apply-templates select="$mainPerson/tia:IMKAD_PostlocatiePersoon" mode="do-correspondant-address" />
							<xsl:text>)</xsl:text>
						</xsl:if>
									...
									<tr>
										...
										<td>
											<xsl:attribute name="class">
												...
												<xsl:value-of select="$chosenNestingDepth" />
											</xsl:attribute>
				...
				<tr>
					...
					<td>
						<xsl:attribute name="class">
							...
							<xsl:choose>
								<xsl:when test="($actingInPrivate = 'true' and $numberOfManagers = 1 and $numberOfRelatedPersonsInAllManager = 1 and $onlyPersonWithBulletInParty = 'false')
												or ($actingInPrivate = 'false' and $onlyPersonWithBulletInParty = 'false')">
