	...
	<xsl:template match="tia:IMKAD_ZakelijkRecht" mode="do-right">
		...
		<xsl:choose>
			...
			<xsl:when test="translate($aardVerkregen, $upper, $lower) = 'eigendom belast met zakelijk recht als bedoeld in art.5,lid 3,onder b, van de belemmeringenwet privaatrecht'">
				...
				<xsl:text> in artikel 5 lid 3 onder b van de Belemmeringenwet Privaatrecht zoals deze luidde tot &#x00E9;&#x00E9;n januari negentienhonderdtwee&#x00EB;nnegentig, met betrekking tot</xsl:text>
			</xsl:when> 							 
			...
			<xsl:when test="translate($aardVerkregen, $upper, $lower) = 'erfpacht'"> 
				<xsl:text>het recht van erfpacht met betrekking tot</xsl:text>
			</xsl:when>						          
