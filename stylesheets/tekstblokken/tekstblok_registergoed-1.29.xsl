...
<!--
*********************************************************
Stylesheet: tekstblok_registergoed_levering.xsl
Version: 1.28 (AA-4303)
...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://xml.apache.org/xalan/java" exclude-result-prefixes="tia kef xsl java" version="1.0">
	...
	**** PARCEL *******************************************************************************************
	...
	<xsl:template match="tia:IMKAD_ZakelijkRecht[tia:IMKAD_Perceel]" mode="do-registered-object">
		...
		<!-- Current parcel size -->
		<xsl:choose>
			<xsl:when test="tia:IMKAD_Perceel/tia:tia_Tekstkeuze[tia:tagNaam='k_EenheidGrootte']/tia:tekst='meter'">
				<xsl:apply-templates select="tia:IMKAD_Perceel/tia:grootte" mode="do-parcel-size-meters"/>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:if test="$sameObjects and $RegistergoedTonenPerPerceel='false'">
			<xsl:for-each select="$sameObjects/tia:grootte">
				...
				<xsl:choose>
					<xsl:when test="../tia:tia_Tekstkeuze[tia:tagNaam='k_EenheidGrootte']/tia:tekst='meter'">
						<xsl:apply-templates select="." mode="do-parcel-size-meters"/>
					</xsl:when>
					<xsl:otherwise>
	...
	**** matching template ********************************************************************************
	-->
	<xsl:template match="tia:grootte" mode="do-parcel-size-meters">
		<!-- example: 2.045.000 m2 = tweemiljoen vijfenveertigduizend vierkante meter (2.045.000 m2)  -->
		<xsl:variable name="meters" select="floor(.)"/>
		<xsl:value-of select="kef:convertNumberToText(string($meters))"/>
		<xsl:text> vierkante meter</xsl:text>
		<xsl:text> (</xsl:text>
		<xsl:value-of select='format-number($meters, "###.###")'/>
		<xsl:text> m&#178;</xsl:text>
		<xsl:text>)</xsl:text>
	</xsl:template>
	<!--
			...
			<xsl:when test="normalize-space($indication) = '3'">
				...
				<xsl:choose>
					...
					<xsl:when test="count(tia:IMKAD_OZLocatie) > 1">
						...
						<xsl:text> </xsl:text>
						<xsl:value-of select="java:nl.kadaster.kpk.server.service.impl.transformer.AddressSortUtilEvenOneven.doParcelAddresses($ozAddresses)"/>
					</xsl:when>
				</xsl:choose>
			</xsl:when>
		</xsl:choose>
