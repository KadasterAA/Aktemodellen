	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_AangebodenStuk" mode="do-statement-of-equivalence">
		...
		<p>
			...
			<xsl:choose>
				<xsl:when test="contains(translate($declarerPlace, $upper, $lower), '{gemeente}')">
					<xsl:value-of select="concat(substring-before($declarerPlace, '{'), tia:tia_Verklaarder/tia:gemeente, substring-after($declarerPlace, '}'))"/>
				</xsl:when>
				<xsl:otherwise>
			...
			<xsl:choose>
				<xsl:when test="((translate($includeTime, $upper, $lower) = 'false' or not(tia:tia_TijdOndertekening) or normalize-space(tia:tia_TijdOndertekening) = '' or normalize-space($time) = '') and normalize-space($verklaringWvg) = '')">
					<xsl:text> dat dit afschrift een volledige en juiste weergave is van de inhoud van het stuk waarvan het een afschrift is.</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>:</xsl:text>
				</xsl:otherwise>
				...
				<tbody>
					...
					<xsl:if test="translate($includeTime, $upper, $lower) = 'true' and tia:tia_TijdOndertekening and normalize-space(tia:tia_TijdOndertekening) != '' and normalize-space($time) != ''">
						<tr>
							...
							<td>
								<xsl:value-of select="concat(substring-before($time, '{'), $Signiture_Time_STRING, ' (' , $Signiture_Time_TIME, ' uur) ', substring-after($time, '}'))"/>
				    			<xsl:choose>
