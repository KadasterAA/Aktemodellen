...
Version: 1.30 (AA-6057, AA-6059)
Naam van de keuzetekst 'k_VerklaringWvg' aangepast naar 'k_VerklaringOmgevingsWet' omwille van de Levering en Verdeling.
*********************************************************
Description:
...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="tia kef xsl" version="1.0">
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_AangebodenStuk" mode="do-statement-of-equivalence">
		...
		<xsl:variable name="verklaringomgevingswet">
			<xsl:choose>
				<xsl:when test="tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaringomgevingswet']">
					<xsl:choose>
						<xsl:when test="translate(tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaringomgevingswet']/tia:tekst, $upper, $lower) = 'false'">
							<xsl:value-of select="normalize-space($keuzeteksten/*/tia:TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaringomgevingswet']/tia:tekst[1])"/>
						</xsl:when>
						<xsl:when test="translate(tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaringomgevingswet']/tia:tekst, $upper, $lower) = 'true'">
							<xsl:value-of select="normalize-space($keuzeteksten/*/tia:TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaringomgevingswet']/tia:tekst[2])"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaringomgevingswet']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
								translate(normalize-space($keuzeteksten/*/tia:TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaringomgevingswet']/tia:tekst[translate(normalize-space(.), $upper, $lower) =
								translate(normalize-space(current()/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaringomgevingswet']/tia:tekst), $upper, $lower)]), $upper, $lower)]"/>
							<xsl:text>.</xsl:text>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
		...
		<p>
			...
			<xsl:choose>
				<xsl:when test="not(tia:heeftVerklaarder/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaarderwaarneming'])">
					...
					<xsl:if test="$woonplaats">						
						<xsl:if test="tia:heeftVerklaarder/tia:gemeente and not(tia:heeftVerklaarder/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_komma']/tia:tekst = 'false')">
							<xsl:text>,</xsl:text>
						</xsl:if>
						<xsl:text> </xsl:text>
						<xsl:choose>
					...
					<xsl:choose>
						...
						<xsl:when test="translate(tia:heeftVerklaarder/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaarderwaarneming']/tia:tekst, $upper, $lower) = 'vacant kantoor'">
							...
							<xsl:if test="tia:heeftVerklaarder/tia:isWaarnemerVoor">
								...
								<xsl:if test="tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:standplaats">									
									<xsl:if test="tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:gemeente and not(tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_komma']/tia:tekst = 'false')">
										<xsl:text>,</xsl:text>
									</xsl:if>
									<xsl:text> </xsl:text>
									<xsl:choose>
							...
							<xsl:choose>
								<xsl:when test="tia:heeftVerklaarder/tia:isVacatureWaarnemerVoor">
									...
									<xsl:if test="tia:heeftVerklaarder/tia:isVacatureWaarnemerVoor/tia:standplaats">										
										<xsl:if test="tia:heeftVerklaarder/tia:isVacatureWaarnemerVoor/tia:gemeente and not(tia:heeftVerklaarder/tia:isVacatureWaarnemerVoor/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_komma']/tia:tekst = 'false')">
											<xsl:text>,</xsl:text>
										</xsl:if>
										<xsl:text> </xsl:text>
										<xsl:choose>
								...
								<xsl:otherwise>
									...
									<xsl:if test="tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:isVacatureWaarnemerVoor/tia:standplaats">										
										<xsl:if test="tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:isVacatureWaarnemerVoor/tia:gemeente and not(tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:isVacatureWaarnemerVoor/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_komma']/tia:tekst = 'false')">
											<xsl:text>,</xsl:text>
										</xsl:if>
										<xsl:text> </xsl:text>
										<xsl:choose>
						...
						<xsl:when test="translate(tia:heeftVerklaarder/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_verklaarderwaarneming']/tia:tekst, $upper, $lower) = 'niet vacant kantoor'">
							...
							<xsl:if test="tia:heeftVerklaarder/tia:isWaarnemerVoor">
								...
								<xsl:if test="tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:gemeente and tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:standplaats and not(tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_komma']/tia:tekst = 'false')">
									<xsl:text>,</xsl:text>
								</xsl:if>
								<xsl:if test="tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:standplaats">									
									<xsl:if test="tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:gemeente and not(tia:heeftVerklaarder/tia:isWaarnemerVoor/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_komma']/tia:tekst = 'false')">
			...
			<xsl:choose>
				<xsl:when test="(not(tia:tia_TijdOndertekening) or normalize-space(tia:tia_TijdOndertekening) = '') and normalize-space($verklaringomgevingswet) = ''">
