			...
			<xsl:when test="$capacityVariant = '2'">
				...
				<xsl:choose>
					<xsl:when test="$numberOfPersons = 1">
						...
						<xsl:value-of select="concat(substring-before($capacityTextVariant2Plurality, 'zijn/haar/hun'), $gender, substring-after($capacityTextVariant2Plurality, 'zijn/haar/hun'))" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat(substring-before($capacityTextVariant2, 'zijn/haar/hun'), 'hun', substring-after($capacityTextVariant2, 'zijn/haar/hun'))" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
