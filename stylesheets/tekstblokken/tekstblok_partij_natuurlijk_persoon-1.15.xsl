...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
	...
	*********************************************************
	...
	Params: maxColspan - maximal colspan in table
			...
			forceSinglePerson - forces rendering of single natural person, regardless of the XML structure
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_Persoon" mode="do-party-natural-person">
		...
		<xsl:param name="forceSinglePerson" select="'false'"/>
		<xsl:param name="eachPersonInSingleParty" select="'false'"/>

		<xsl:choose>
			<xsl:when test="tia:GerelateerdPersoon and $forceSinglePerson = 'false'">
			...
			<xsl:otherwise>
				<xsl:apply-templates select="." mode="do-party-natural-person-single">
					...
					<xsl:with-param name="anchorName" select="$anchorName"/>
					<xsl:with-param name="personTerminator" select="$personTerminator"/>
					<xsl:with-param name="includePartyLetter" select="$includePartyLetter"/>
					<xsl:with-param name="partyNumberingStyle" select="$partyNumberingStyle"/>
					<xsl:with-param name="personNumberingStyle" select="$personNumberingStyle"/>
					<xsl:with-param name="eachPersonInSingleParty" select="$eachPersonInSingleParty"/>
				</xsl:apply-templates>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_Persoon" mode="do-party-natural-person-single">
		...
		<xsl:param name="includePartyLetter"/>
		<xsl:param name="partyNumberingStyle"/>
		<xsl:param name="personNumberingStyle"/>
		<xsl:param name="eachPersonInSingleParty"/>
		...
		<tr>
			...
			<xsl:if test="$eachPersonInSingleParty = 'false' and $numberOfPersons > 0 and 
							((not(translate($nestedParty, $upper, $lower) = 'true' and count(preceding-sibling::tia:IMKAD_Persoon) = 0)) 
							   or (translate($nestedParty, $upper, $lower) = 'true' and count(preceding-sibling::tia:IMKAD_Persoon) = 0) )">
				<td class="number" valign="top">
					<xsl:choose>
						<xsl:when test="translate($nestedParty, $upper, $lower) = 'true'">
							<xsl:choose>
		...
		<xsl:if test="../tia:vofCvMs and count(following-sibling::tia:IMKAD_Persoon) = 0">
			<tr>
				<td class="number" valign="top">
					<xsl:text>&#xFEFF;</xsl:text>
				</td>
						...
						<xsl:when test="translate($nestedParty, $upper, $lower) = 'true'">
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test="$position != 0">
											<xsl:number value="$start + $position - 1" format="a"/>
										</xsl:when>
										<xsl:otherwise>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_Persoon" mode="do-person-pair-representative-variant-seven">
		...
		<xsl:for-each select="tia:GerelateerdPersoon">
			<tr>
				...
				<td>
					...
					<xsl:apply-templates select="tia:IMKAD_Persoon" mode="do-party-natural-person-common"/>
		...
		<tr>
			...
			<xsl:if test="(count(preceding-sibling::tia:IMKAD_Persoon) + count(following-sibling::tia:IMKAD_Persoon)) != 0 and not($maxColspan >= 2)">
				<td class="number" valign="top">
					<xsl:text>&#xFEFF;</xsl:text>
				</td>
			</xsl:if>
			<td>
				<xsl:if test="($maxColspan >= 2)">
					<xsl:attribute name="colspan">					
						<xsl:value-of select="$maxColspan"/>
					</xsl:attribute>
				</xsl:if>
		...
		<tr>
			...
			<td>
				...
				<ul class="arrow">
					...
					<xsl:if test="tia:tia_ExecuteurVan">
						<li class="arrow">
							<xsl:text> in </xsl:text>
							<xsl:choose>
								<xsl:when test="tia:tia_Gegevens/tia:GBA_Ingezetene/tia:geslacht/tia:geslachtsaanduiding = 'Vrouw' or tia:tia_Gegevens/tia:IMKAD_NietIngezetene/tia:geslacht = 'Vrouw'">
									<xsl:text>haar</xsl:text>
								</xsl:when>
								<xsl:when test="tia:tia_Gegevens/tia:GBA_Ingezetene/tia:geslacht/tia:geslachtsaanduiding = 'Man' or tia:tia_Gegevens/tia:IMKAD_NietIngezetene/tia:geslacht = 'Man'">
									<xsl:text>zijn</xsl:text>
								</xsl:when>
							</xsl:choose>
							<xsl:text> hoedanigheid van executeur in de nalatenschap van </xsl:text>
							<xsl:value-of select="tia:tia_ExecuteurVan"/>
