...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_ZakelijkRecht" mode="do-right">
		...
		<xsl:variable name="numberOfRights" select="number(tia:tia_Aantal_BP_Rechten)" />
		<xsl:variable name="numberOfRightsText" select="kef:convertNumberToText(tia:tia_Aantal_BP_Rechten)" />
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_ZakelijkRecht" mode="do-variant">
		<xsl:variable name="aardVerkregenVariant" select="translate(tia:aardVerkregenVariant, $upper, $lower)"/>
		<xsl:variable name="numberOfRights" select="kef:convertNumberToText(tia:tia_Aantal_BP_Rechten)" />
		...
		<xsl:choose>
			...
			<xsl:when test="$aardVerkregenVariant = 'bp recht'">	
				<xsl:variable name="numberOfAantalBpRechtenvariant" select="tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_aantal_bp_rechtenvariant']/tia:tekst"/>
				<xsl:choose>
					<xsl:when test="number($numberOfAantalBpRechtenvariant) > 1">
						<xsl:text>een</xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:text>het</xsl:text>
				...
				<xsl:text> </xsl:text>
				<xsl:if test="number($numberOfAantalBpRechtenvariant) > 1">
					<xsl:value-of select="kef:convertNumberToText(string(tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_aantal_bp_rechtenvariant']/tia:tekst))"/>
					<xsl:text>tal</xsl:text>
				</xsl:if>
				<xsl:text> </xsl:text>
				<xsl:value-of select="$nadarTeOmschrijvenVarText"/>
