...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
	...
	exclude-result-prefixes="tia xsl exslt xlink"
	version="1.0">
	...
	*********************************************************
	...
	Params: partyNumber - number of the party, default is 1
	...
	<xsl:template match="tia:Partij" mode="do-mortgage-deed-party-name">
		...
		<xsl:choose>
			<xsl:when test="translate(tia:aanduidingPartij, $upper, $lower) = 'aanduiding per persoon'">
				...
				<xsl:variable name="naamHypotheekgever" select="tia:tekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_naamhypotheekgever']"/>
				<xsl:variable name="naamSchuldenaar" select="tia:tekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_naamschuldenaar']"/>
				...
				<xsl:if test="(not($existGevolmachtigde) and $number) or $name">
					...
					<u><xsl:value-of select="$naamSchuldenaar/tia:tekst"></xsl:value-of></u>
					<xsl:text>' en </xsl:text>
					<xsl:if test="$number">
						<xsl:text>de verschenen </xsl:text>
						<xsl:choose>
					...
					<xsl:text>ook te noemen: '</xsl:text>
					<u><xsl:value-of select="$naamHypotheekgever/tia:tekst"></xsl:value-of></u>
					<xsl:text>'</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="contains(translate(tia:aanduidingPartij, $upper, $lower), 'en/of')">
						<xsl:text>'</xsl:text><u><xsl:value-of select="substring-before(tia:aanduidingPartij, ' en/of')"/></u><xsl:text>'</xsl:text>
						<xsl:text> en/of '</xsl:text><u><xsl:value-of select="substring-after(tia:aanduidingPartij, 'en/of ')"/></u><xsl:text>'</xsl:text>
					</xsl:when>
					<xsl:otherwise>
