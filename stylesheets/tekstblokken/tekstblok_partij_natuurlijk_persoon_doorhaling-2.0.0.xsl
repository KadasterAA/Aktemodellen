			...
			<xsl:choose>
				...
				<xsl:when test="($numberOfColums > 2)">
					<td class="level0" colspan="2">
						...
						<xsl:apply-templates select="." mode="do-natural-person"></xsl:apply-templates>
						<xsl:text>, </xsl:text>
						...
						<xsl:apply-templates select="." mode="do-marital-status"></xsl:apply-templates>
						<xsl:text>, wonende te </xsl:text>
						<!-- TEKSTBLOK_WOONADRES -->
						<xsl:apply-templates select="tia:IMKAD_WoonlocatiePersoon" mode="do-address"></xsl:apply-templates>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_Persoon" mode="do-party-natural-person-pair">
		...
		<xsl:choose> 
			...
			<xsl:when test="tia:GerelateerdPersoon[(translate(tia:rol, $upper, $lower) = 'huisgenoot') and (translate(tia:IndGezamenlijkeWoonlocatie, $upper, $lower) ='true')]/tia:IMKAD_Persoon">
		...
		<xsl:choose>
			...
			<xsl:when test="$onlyPersonInParty = 'true'">
				...
				<tr>
					...
					<td class="level0" colspan="2">
						<!-- TEKSTBLOK_BURGERLIJKE_STAAT -->
						<xsl:apply-templates select="." mode="do-marital-status-partners"></xsl:apply-templates>
	...
	<xsl:template match="tia:IMKAD_Persoon" mode="do-party-natural-person-pair-person-two">
		...
		<xsl:choose>
			...
			<xsl:when test="($skipPartyLetterColumn = 'true') and ($numberOfColums > 2) and ($skipPersonLetterColumn = 'true')">
				...
				<td class="level0" colspan="2">
					<!-- TEKSTBLOK_NATUURLIJK_PERSOON -->
					<xsl:apply-templates select="tia:GerelateerdPersoon/tia:IMKAD_Persoon" mode="do-natural-person"></xsl:apply-templates>
					<xsl:text>, </xsl:text>
					<!-- TEKSTBLOK_LEGITIMATIE -->
			...
			<xsl:when test="($skipPartyLetterColumn = 'true') and ($numberOfColums > 2) and ($skipPersonLetterColumn = 'true')">
				...
				<td class="level0" colspan="2">
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:apply-templates select="tia:GerelateerdPersoon/tia:IMKAD_Persoon/tia:IMKAD_WoonlocatiePersoon" mode="do-address"></xsl:apply-templates>
						</xsl:otherwise>
					</xsl:choose>
					<xsl:text>,</xsl:text>
				</td>
