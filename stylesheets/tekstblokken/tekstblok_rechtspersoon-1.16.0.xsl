...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:NHR_Rechtspersoon | tia:vofCvMs" mode="do-legal-person-data">
		...
		<xsl:text>, </xsl:text>
		<!-- Optioneel: tevens (handelend onder de naam): //IMKAD_Persoon/tia_TekstKeuze/tagNaam(k_HandelendOnderNaam)/tekst(tevens) -->
		<xsl:if test="normalize-space(../../tia:tia_TekstKeuze[(tia:tagNaam='k_HandelendOnderNaam')]/tia:tekst) != ''">
			<xsl:value-of select="normalize-space(../../tia:tia_TekstKeuze[(tia:tagNaam='k_HandelendOnderNaam')]/tia:tekst)"/>
			<xsl:text> </xsl:text>
		</xsl:if>
	...
	**** matching template ********************************************************************************
	-->
	<xsl:template match="tia:NHR_Rechtspersoon" mode="do-chamber-of-commerce-data">
		...
		<xsl:variable name="besluitnummer" select="../../tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_besluitnummer']/tia:tekst"/>
		...
		<xsl:if test="translate(tia:rechtsvormSub, $upper, $lower) = 'publiekrechtelijk rechtspersoon' and normalize-space($gegevensOndertekening/tia:naamGemeente) != ''">
			<xsl:text>, ter uitvoering van het besluit</xsl:text>
			<xsl:if test="string-length($besluitnummer) != 0">
				<xsl:text> met nummer </xsl:text>
				<xsl:value-of select="$besluitnummer"/>
			</xsl:if>
			<xsl:text> van het College van Burgemeester en Wethouders van de gemeente </xsl:text>
