...
Version: 9.0.0
-[AA-6057] WVG: modelaanpassing Akte van levering
*********************************************************
Description:
Deed of transfer.
...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" exclude-result-prefixes="tia kef xsl exslt xlink gc" version="1.0">
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten-levering-8.0.xml')"/>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Partij" mode="do-comparitie">
		<xsl:variable name="numberOfPersons" select="count(tia:IMKAD_Persoon[normalize-space(tia:tia_IndGerechtigde) = 'true']) + count(tia:IMKAD_Persoon/tia:GerelateerdPersoon/tia:IMKAD_Persoon[normalize-space(tia:tia_IndGerechtigde) = 'true']) + count(tia:IMKAD_Persoon/tia:GerelateerdPersoon/tia:IMKAD_Persoon/tia:GerelateerdPersoon/tia:IMKAD_Persoon[normalize-space(tia:tia_IndGerechtigde) = 'true'])"/>
		...
		<table cellspacing="0" cellpadding="0">
			<tbody>
				...
				-->
<!--				<xsl:choose>-->
<!--					&lt;!&ndash; If only one person pair, or legal person with warrantors is present - do not create list &ndash;&gt;-->
...
<!--				</xsl:choose>-->
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:IMKAD_AangebodenStuk" mode="do-keuzeblok-verdeling">
		...
		<!-- AA-8784 -->
		<xsl:variable name="gedeeldeHuwelijkseStaatTekst">
			<xsl:choose>
				<xsl:when test="$acquirerParty/tia:IMKAD_Persoon/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_burgerlijkestaattekstpartners']/tia:tekst">
					<xsl:value-of select="$acquirerParty/tia:IMKAD_Persoon/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_burgerlijkestaattekstpartners']/tia:tekst"/>
				</xsl:when>
				<xsl:when test="$acquirerParty/tia:IMKAD_Persoon/tia:GerelateerdPersoon/tia:IMKAD_Persoon/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_burgerlijkestaattekstpartners']/tia:tekst">
					<xsl:value-of select="$acquirerParty/tia:IMKAD_Persoon/tia:GerelateerdPersoon/tia:IMKAD_Persoon/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_burgerlijkestaattekstpartners']/tia:tekst"/>
				</xsl:when>
				<xsl:otherwise>false</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="partyMariatalStatusText" select="$acquirerParty/tia:IMKAD_Persoon/tia:GerelateerdPersoon/tia:IMKAD_Persoon/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_burgerlijkestaattekstpartners']/tia:tekst"/>
				...
				<xsl:choose>
					<xsl:when test="$numberOfBuyers = 2">
						<xsl:if test="$acquirerParty">
							<!-- Als er twee verkrijger-personen zijn, die in (beperkte) gemeenschap van goederen getrouwd zijn of zonder het maken van partnerschapsvoorwaarden geregistreerd partner zijn, -->
							<xsl:if test="$gedeeldeHuwelijkseStaatTekst = '5' or
											$gedeeldeHuwelijkseStaatTekst = '6' or
											$gedeeldeHuwelijkseStaatTekst = '7' or
											$gedeeldeHuwelijkseStaatTekst = '10' or
											$gedeeldeHuwelijkseStaatTekst = '11' or
											$gedeeldeHuwelijkseStaatTekst = '12' or
											$gedeeldeHuwelijkseStaatTekst = '15' or
											$gedeeldeHuwelijkseStaatTekst = '18' or
											$gedeeldeHuwelijkseStaatTekst = '21' or
											$gedeeldeHuwelijkseStaatTekst = '22'">
								<xsl:text>, gezamenlijk</xsl:text>
							</xsl:if>
						</xsl:if>
						<!-- Als er twee verkrijger-personen zijn, dan wordt de tekst "ieder voor de onverdeelde helft" vermeld. -->
