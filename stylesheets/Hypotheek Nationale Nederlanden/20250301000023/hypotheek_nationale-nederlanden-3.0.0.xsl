...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common" xmlns:kef="nl.kadaster.xslt.XslExtensionFunctions" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" exclude-result-prefixes="tia xsl exslt xlink gc" version="1.0">
	...
	<xsl:include href="keuzeblok_partijnamen_hypotheekakte_nationale-nederlanden-2.0.0.xsl"/>
	...
	<xsl:variable name="keuzeteksten" select="document('keuzeteksten_hypotheek_nationale-nederlanden-3.0.0.xml')"/>
	...
	*********************************************************
	...
	Description: Hypotheek Nationale Nederlanden.
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Bericht_TIA_Stuk" mode="do-deed">
		...
		<p>
			<xsl:text>De comparanten, handelend als voormeld, verklaarden dat tussen hen een overeenkomst van geldlening met hypotheekstelling en inpandgeving is gesloten, welke overeenkomst is omschreven in de door de schuldenaar getekende offerte van schuldeiseres onder nummer </xsl:text>
			<xsl:value-of select="tia:IMKAD_AangebodenStuk/tia:tia_TekstKeuze[tia:tagNaam='k_Offertenummer']/tia:tekst"/>
			<xsl:text>, hierna te noemen ‘Offerte’. Ter uitvoering van die overeenkomst zal door schuldeiseres een som geld aan de schuldenaar ter beschikking worden gesteld en zullen door de hypotheekgever een recht van hypotheek en pandrechten ten behoeve van schuldeiseres worden gevestigd, een en ander zoals in deze akte omschreven.</xsl:text>
		</p>
		<h3>
			<xsl:text>BEDRAG GELDLENING (Offertenummer: </xsl:text>
			<xsl:value-of select="tia:IMKAD_AangebodenStuk/tia:tia_TekstKeuze[tia:tagNaam='k_Offertenummer']/tia:tekst"/>
			<xsl:text>)</xsl:text>
		</h3>
		<p>
			<xsl:text>De schuldenaar verklaart heden van schuldeiseres ter leen te hebben ontvangen en aan deze daarom een bedrag van </xsl:text>
		...
		<p>
			<xsl:text>De hypotheekgever verleent tot zekerheid voor de betaling van al hetgeen schuldeiseres nu of te eniger tijd te vorderen mocht hebben van schuldenaar onder of in verband met vorenbedoelde overeenkomst van geldlening een recht van hypotheek </xsl:text>
			...
			<xsl:text> in rang, en wel op het registergoed zoals hierna vermeld voor een bedrag van </xsl:text>
			<xsl:call-template name="amountText">
			...
			<xsl:text> te vermeerderen met veertig procent (40%) voor rente, boeten en kosten onder of in verband met vorenbedoelde overeenkomst van geldlening, dus tot een eindbedrag </xsl:text>
			<xsl:call-template name="amountText">
		...
		<xsl:if test="translate(tia:IMKAD_AangebodenStuk/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_eindbedrag']/tia:tekst, $upper, $lower) = 'true'">
			<p>
				<xsl:text>Voormeld eindbedrag kan in zijn geheel worden verhaald op elk van de in deze akte omschreven registergoederen.</xsl:text>
			</p>
		</xsl:if>
		<p>
			<xsl:text>Op deze geldlening en hiervoor respectievelijk hierna te noemen hypotheek- en pandverlening zijn de bepalingen in de Offerte en de overige bepalingen in 'De voorwaarden bij uw Nationale-Nederlanden Hypotheek' en 'de Algemene Bankvoorwaarden' - genoemd in de Offerte - van toepassing.</xsl:text>
		</p>
		<h3>
			<xsl:text>ZEKERHEDEN</xsl:text>
		</h3>
		<p>
		...
		<p>
			...
			<xsl:if test="translate(tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[not(tia:aanduidingHypotheek) or normalize-space(tia:aanduidingHypotheek) = '']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_registergoederentezamen']/tia:tekst, $upper, $lower) = 'true'">
				<xsl:text>tezamen met alle eventuele overige in deze akte te noemen registergoederen </xsl:text>
			</xsl:if>
		...
		translate(tia:IMKAD_AangebodenStuk/tia:tia_TekstKeuze[translate(tia:tagNaam, $upper, $lower) = 'k_lastendepot']/tia:tekst, $upper, $lower) = 'true'">
			<h3>
				<xsl:text>OVERBRUGGING EN/OF DEPOTS</xsl:text>
			</h3>
		</xsl:if>
		<xsl:if test="tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[translate(tia:aanduidingHypotheek, $upper, $lower) = 'overbruggingshypotheek']">
			...
			<p>
				...
				<xsl:text> hypotheek wordt gevestigd op:</xsl:text>
			</p>
				<a name="hyp3.rights" class="location">&#160;</a>
				<xsl:apply-templates select="." mode="do-rights">
			...
			<p>
				<xsl:text>in deze akte </xsl:text>
				<xsl:if test="translate(tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[translate(tia:aanduidingHypotheek, $upper, $lower) = 'overbruggingshypotheek']/tia:tekstkeuze[translate(tia:tagNaam, $upper, $lower) = 'k_registergoederentezamen']/tia:tekst, $upper, $lower) = 'true'">
					<xsl:text>tezamen met alle eventuele overige in deze akte te noemen registergoederen </xsl:text>
				</xsl:if>
				<xsl:text>te noemen 'het registergoed'.</xsl:text>
			</p>
						...
						<td valign="top">
							...
							<xsl:choose>
								<xsl:when test="tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[translate(tia:aanduidingHypotheek, $upper, $lower) = 'overbruggingshypotheek']/tia:naamHypotheekhouder">
									<xsl:value-of select="tia:IMKAD_AangebodenStuk/tia:StukdeelHypotheek[translate(tia:aanduidingHypotheek, $upper, $lower) = 'overbruggingshypotheek']/tia:naamHypotheekhouder"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>schuldeiseres</xsl:text>
								</xsl:otherwise>
							</xsl:choose>
							<xsl:text>;</xsl:text>
						</td>
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Partij" mode="do-comparitie">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<p style="margin-left:30px">
					<xsl:text>hierna ook te noemen ’schuldeiseres’, alsmede haar rechtverkrijgenden onder algemene en onder bijzondere titel. Overal waar in deze akte of in de Voorwaarden (zoals hierna in deze akte gedefinieerd) gesproken wordt van schuldeiseres, zal na overgang hierin worden gelezen haar rechtsopvolger.</xsl:text>
				</p>
				<p style="margin-left:30px">
					<xsl:text>Van de hier bedoelde volmacht door de schuldeiseres is mij, notaris, genoegzaam gebleken.</xsl:text>
				</p>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
