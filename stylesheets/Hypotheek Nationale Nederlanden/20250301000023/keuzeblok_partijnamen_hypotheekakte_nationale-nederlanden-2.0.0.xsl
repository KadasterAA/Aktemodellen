...
<!--
*********************************************************
Stylesheet: keuzeblok_partijnamen_hypotheekakte_nationale-nederlanden.xsl
Version: 2.0.0
- [AA-7713] Genderneutraal maken Hypotheekakten - Lloyds, LOT, NN, Vista, Particuliere hypotheek
*********************************************************
Description:
KEUZEBLOK PARTIJNAMEN HYPOTHEEKAKTE - Nationale Nederlanden
...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:tia="http://www.kadaster.nl/schemas/KIK/TIA_Algemeen" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common" xmlns:xlink="http://www.w3.org/1999/xlink" exclude-result-prefixes="tia xsl exslt xlink" version="1.0">
	...
	**** matching template ********************************************************************************
	...
	<xsl:template match="tia:Partij" mode="do-keuzeblok-partijnamen-hypotheekakte">
		...
		<xsl:variable name="debtor" select="'schuldenaar'"/>
		...
		<xsl:choose>
			<!-- Optie 1: schuldenaar en/of hypotheekgever' -->
			<xsl:when test="translate(tia:aanduidingPartij, $upper, $lower) = 'schuldenaar en/of hypotheekgever'">
				...
				<xsl:text> 'schuldenaar' en/of 'hypotheekgever'</xsl:text>
			</xsl:when>
			...
			<xsl:when test="translate(tia:aanduidingPartij, $upper, $lower) = 'aanduiding per persoon'">
				...
				<xsl:if test="(not($existGevolmachtigde) and $number) or $name">
					...
					<xsl:text> 'schuldenaar' en </xsl:text>
					<xsl:if test="$number">
						<xsl:text>de verschenen </xsl:text>
						<xsl:choose>
